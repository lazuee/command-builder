...
import type { CommandSubcommandBuilder, CommandSubcommandGroupBuilder } from './CommandSubcommands';
...
const descriptionPredicate = s.string.lengthLessThanOrEqual(100);
...
export function validateRequiredParameters(name: string, description: string, options: ToAPIApplicationCommandOptions[]) {
  validateName(name);
...
export function assertReturnOfBuilder<T extends ApplicationCommandOptionBase | CommandSubcommandBuilder | CommandSubcommandGroupBuilder>(
  input: unknown,
...
export const localizationMapPredicate = s.object<LocalizationMap>(Object.fromEntries(Object.values(Locale).map(locale => [locale, s.string.nullish]))).strict.nullish;
